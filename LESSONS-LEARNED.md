...
| 28 | Neo4j Community: `dbms.backup.enabled` setting | `Unrecognized setting. No declared setting with name: dbms.backup.enabled` — Container restart loop | Enterprise-only setting; entfernen aus compose, oder `NEO4J_server_config_strict__validation_enabled=false` |
| 29 | CIFS bind-mount in unprivileged LXC nicht beschreibbar | NAS-Mount mit `uid=0,gid=0` → in unpriv LXC erscheint als "nobody" → cp scheitert mit Permission denied | Mount mit `uid=100000,gid=100000` (LXC root = host UID 100000) |
| 30 | SSH heredoc mit eingebetteten Quotes bricht ab | `warning: here-document at line 1 delimited by end-of-file (wanted SCRIPT')` | Datei lokal vorbereiten + `pct push` (oder `scp` + `pct push`) statt heredoc-über-ssh |
| 31 | Neo4j Browser über HTTPS kann nicht zu `bolt://` verbinden | Mixed content blocking — moderner Browser blockt `ws://` von HTTPS-Seite. `Failed to fetch` Fehler | Bolt TLS aktivieren: `NEO4J_dbms_ssl_policy_bolt_enabled=true` + Cert mounten als `/ssl/bolt/{private.key,public.crt}` + Connect-URL `neo4j+s://` oder `bolt+ssc://` |
